...
|---|---|---|---|---|
| rig.gladiator_placeholder | layered character rig | `PlaceholderRig` v3: 3-tone anatomy, face, per-slot armour overlays, tier-tinted per-class weapon drawings | Full modular layer set per charter §12 (Body/Head/Hair/…/MainWeapon/Effects) on one shared skeleton; attachment points main_hand, off_hand, head, back, shield; anims: Idle, Walk, Run, Jump, AttackLight, AttackHeavy, AttackSpecial, Shoot, Cast, Block, Parry, Hit, CriticalHit, Stunned, Taunt, Victory, Death | placeholder |
...
| arena.gravelmaw.backdrop | background set | `ArenaVisual` v2: sky/sun, drawn crowd tiers (cheering figures), brick wall, barred pen gates, columns, lit sand oval | layered parallax-capable backdrop, 1920x1080 safe | placeholder |
